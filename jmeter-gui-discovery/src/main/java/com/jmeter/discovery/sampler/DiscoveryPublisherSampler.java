package com.jmeter.discovery.sampler;
...
import com.jmeter.discovery.control.gui.DiscoveryActivemqPublisherGui;
import com.thoughtworks.xstream.XStream;

public class DiscoveryPublisherSampler extends BaseJMSSampler implements
		TestStateListener {
	...
	private static final String ZOOKEEPER_USER = "zookeeper.user";
	...
	// Cache for object-message, only used when parsing from a file because in
	// text-area
	...
	public DiscoveryPublisherSampler() {
	}
	...
	private void initClient() throws JMSException, NamingException {
		...
		sysProperty.setProperty("zookeeper.url",getZookeeperUrl());
		sysProperty.setProperty("zookeeper.password", getZookeeperPassword());
		log.info("*******getZookeeperPassword**************"+getZookeeperPassword());
		log.info("*******getZookeeperUrl*******************"+getZookeeperUrl());
		publisher = new Publisher(getUseJNDIPropertiesAsBoolean(),
				getJNDIInitialContextFactory(), getProviderUrl(),
				getConnectionFactory(), getDestination(), isUseAuth(),
				getUsername(), getPassword(), isDestinationStatic());
	...
	public SampleResult sample() {
		...
		try {
			Map<String, Object> msgProperties = getJMSProperties()
			...
			for (int idx = 0; idx < loop; idx++) {
				if (DiscoveryActivemqPublisherGui.TEXT_MSG_RSC.equals(type)) {
				...
				} else if (DiscoveryActivemqPublisherGui.MAP_MSG_RSC.equals(type)) {
					Map<String, Object> m = getMapContent();
					Message msg = publisher.publish(m, getDestination(),
							msgProperties, deliveryMode, priority, expiration);
					Utils.messageProperties(propBuffer, msg);
				} else if (DiscoveryActivemqPublisherGui.OBJECT_MSG_RSC.equals(type)) {
					Serializable omsg = getObjectContent();
					Message msg = publisher.publish(omsg, getDestination(),
							msgProperties, deliveryMode, priority, expiration);
					Utils.messageProperties(propBuffer, msg);
				} else if (DiscoveryActivemqPublisherGui.BYTES_MSG_RSC.equals(type)) {
					byte[] bmsg = getBytesContent();
					Message msg = publisher.publish(bmsg, getDestination(),
							msgProperties, deliveryMode, priority, expiration);
					Utils.messageProperties(propBuffer, msg);
				} else {
	...
	private String getMessageContent() {
		if (getConfigChoice().equals(DiscoveryActivemqPublisherGui.USE_FILE_RSC)) {
			...
			if (file_contents == null
					|| !Objects.equals(cachedFileName, getInputFile())) {
				cachedFileName = getInputFile();
				file_contents = getFileContent(getInputFile());
			}
			return file_contents;
		} else if (getConfigChoice().equals(DiscoveryActivemqPublisherGui.USE_RANDOM_RSC)) {
			...
			String fname = FSERVER.getRandomFile(getRandomPath(),
					new String[] { ".txt", ".obj" }).getAbsolutePath();
	...
	/**
	 * This method will load the contents for the JMS Object Message. The
	 * contents are either loaded from file (might be cached), random file or
	 * from the GUI text-area.
	 * 
	...
	private Serializable getObjectContent() {
		if (getConfigChoice().equals(DiscoveryActivemqPublisherGui.USE_FILE_RSC)) {
			...
			// Maybe we should consider creating a global cache for the
			// random files to make JMeter more efficient.
			final String fname = FSERVER.getRandomFile(getRandomPath(),
					new String[] { ".txt", ".obj" }).getAbsolutePath();
	...
	/**
	 * This method will load the contents for the JMS BytesMessage. The contents
	 * are either loaded from file (might be cached), random file
	 * 
	...
	private byte[] getBytesContent() {
		if (getConfigChoice().equals(DiscoveryActivemqPublisherGui.USE_FILE_RSC)) {
			...
			final String fname = FSERVER.getRandomFile(getRandomPath(),
					new String[] { ".dat" }).getAbsolutePath();
	...
	/**
	 * Try to load an object from a provided file, so that it can be used as
	 * body for a JMS message. An {@link IllegalStateException} will be thrown
	 * if loading the object fails.
	 * 
	 * @param path
	 *            Path to the file that will be serialized
	...
	private static Serializable getFileObjectContent(final String path) {
		...
		try {
			...
			readObject = (Serializable) xstream
					.fromXML(inputStream, readObject);
		} catch (Exception e) {
			log.error(e.getLocalizedMessage(), e);
			throw new IllegalStateException(
					"Unable to load object instance from file", e);
		} finally {
			JOrphanUtils.closeQuietly(inputStream);
		}
		return readObject;
	...
	/**
	 * Try to load an object via XStream from XML text, so that it can be used
	 * as body for a JMS message. An {@link IllegalStateException} will be
	 * thrown if transforming the XML to an object fails.
	 * 
	 * @param xmlMessage
	 *            String containing XML text as input for the transformation
	 * @return Serialized object instance
	 */
	private static Serializable transformXmlToObjectMessage(
		...
		try {
			...
			log.error(e.getLocalizedMessage(), e);
			throw new IllegalStateException(
					"Unable to load object instance from text", e);
		}
		return readObject;
	}
	...
	private static final String USE_FILE_LOCALNAME = JMeterUtils
			.getResString(DiscoveryActivemqPublisherGui.USE_FILE_RSC);
	private static final String USE_RANDOM_LOCALNAME = JMeterUtils
			.getResString(DiscoveryActivemqPublisherGui.USE_RANDOM_RSC);
	...
	public String getConfigChoice() {
		...
		if (config.equals(USE_FILE_LOCALNAME)
				|| config.equals(DiscoveryActivemqPublisherGui.USE_FILE_RSC)) {
			return DiscoveryActivemqPublisherGui.USE_FILE_RSC;
		}
		if (config.equals(USE_RANDOM_LOCALNAME)
				|| config.equals(DiscoveryActivemqPublisherGui.USE_RANDOM_RSC)) {
			return DiscoveryActivemqPublisherGui.USE_RANDOM_RSC;
		}
	...
	/**
	 * set the input file for the ActiveMqPublisher
	 * 
	 * @param file
	 *            input file for the ActiveMqPublisher
	 */
	...
	public void setZookeeperUrl(String s) {
		setProperty(ZOOKEEPER_URL, s, "localhost:2181");
	}

	public String getZookeeperUrl() {
		return getPropertyAsString(ZOOKEEPER_URL, "localhost:2181");
	}

	public void setZookeeperPassword(String s){
		setProperty(ZOOKEEPER_PASSWORD, s, "admin");
	}
	public String getZookeeperPassword(){
		return getPropertyAsString(ZOOKEEPER_PASSWORD, "admin");
	}

	public void setZookeeperUser(String s){
		setProperty(ZOOKEEPER_USER, s, "admin");
	}
	public String getZookeeperUser(){
		return getPropertyAsString(ZOOKEEPER_USER, "admin");
	}
	...
	public JMSProperties getJMSProperties() {
		...
		if (o instanceof Arguments) {
			jmsProperties = Utils
					.convertArgumentsToJmsProperties((Arguments) o);
		} else {
